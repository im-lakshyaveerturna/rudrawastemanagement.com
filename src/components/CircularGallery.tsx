...
class App {
  ...
  onTouchUp(e: any) {
    ...
    if (distX < 10 && distY < 10 && this.onImageClick && this.medias && this.medias.length) {
      const rect = this.gl.canvas.getBoundingClientRect();
      const localX = endX - rect.left; // pixel within canvas
      const worldX = (localX / this.screen.width) * this.viewport.width - this.viewport.width / 2;

      let closest = { idx: 0, dist: Number.POSITIVE_INFINITY };
      for (let i = 0; i < this.medias.length; i++) {
        const mx = this.medias[i].plane.position.x; // world x
      ...
      const originalCount = this.mediasImages.length / 2 || 1;
      const baseIndex = closest.idx % originalCount;
      this.onImageClick(baseIndex);
    }
    this.onCheck();
  }
  onWheel(e: any) {
